...
*** =UsageBehaviorRule= --- Direction/Usage
  ...
  * +rule/designation --- 1 of:+
    * +unspecified/bidirectional+
    * ++s only+
    * +-s only+
    * +bidirectional turning lane+
    * +restricted/no-traffic+
  * +longitudinal extent =[s0, s1]=+
  * +/Should "no applicable record found" equate to "unspecified"?/+
...
*** =RightOfWayRule= --- Stopping and Yielding
  ...
  * rule
    * static
      * =StaticRightOfWayRule= <--- NEED A GOOD NAME
