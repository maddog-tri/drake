...
\left(\begin{array}{c} G_{xy}\\ l_\text{max}G_z \end{array}\right)
\]
defines a 3-d /reference curve/ $W(p)_\text{SEG}$
...
With $\mathbf{R}$, we can define the =Segment= /surface/ $W(p,r)_\text{SEG}$:
\begin{align*}
(x,y,z) = W(p,r)_\text{SEG} = \left(
\begin{array}{c}
   G_{xy}(p_\text{SEG})\\
   G_z(p_\text{SEG})
\end{array} \right) +
...
**** Volumetric
With $\mathbf{R}$, we can define the world transform $W$:
